...
def dollar_adjusting(ticker_info):
    print(ticker_info)
    ticker = ticker_info['name']
    path = ticker_info['path']
    yearpath = ticker_info['year_path']
    for inn in [ticker]:
        print(inn)
        for z in range(len(yearpath)):
            print(f'{ticker}_{yearpath[z]}')
            newp = f'./docker_storage/Raw_Data_New/{ticker}_{yearpath[z]}/'
            path = newp
            print('------------------------------------')
    #         print(inn)
