...
#-------------------------------------------------------------------------
        ...
        temp.to_feather(f'./docker_storage/Tick_Data/{tic}-|{start_date}_{end_date}|-Tick-Data.ftr')
        ...
        for barsperday in [10]:
            ...
            dolDF.to_feather(f'./docker_storage/{tic}_dolDF_const_{barsperday}_BarsPerDay.ftr')
            gc.collect()
        gc.collect()
    else:
        print(tic)
...
#-------------------------------------------------------------------------
        ...
        temp.reset_index(drop=True, inplace=True)
        temp.to_feather(f'./docker_storage/{tic}-{start_date}_{end_date}-Tick-Data.ftr')
        ...
        for barsperday in [10]:
            ...
            gc.collect()
        gc.collect()
    ticker_info['resample-path'] = f"./docker_storage/{tic}_dolDF_const_{barsperday}_BarsPerDay.ftr"
    ticker_info["impute-path"] = f"./docker_storage/{tic}-|{start_date}_{end_date}|-Tick-Data.ftr"
    return ticker_info