...
def make_all_features(tic):
    secDict=dict()
    print('aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa11111111111111111111')
    try:
        print('------------------------------------------------------------------')
        print(tic)
        ...
        print('collsss start1:',secDict[tic].columns)
        ...
        print('collsss start:',secDict[tic].columns)
        ...
        print('after dropping na, length of df is 1:',len(secDict[tic]))
        ...
        print('columns in secdict',secDict[tic].columns)
    ...
    # secDict[tic].drop(columns=['feature_UB', 'feature_LB', 'feature_EMA10', 'feature_EMA21', 'feature_EMA55', 'feature_macd', 'feature_macdsignal', 'feature_macdhist'], axis=1, inplace=True)
        ...
        print('columns in secdict1',secDict[tic].columns)
        cols=secDict[tic].columns
        cols=[s for s in cols if 'ffd_' in s]
        print('cols value:',cols)
        ...
        secDict[tic]['close+100']=secDict[tic]['C'].shift(-(100))
        print(len(secDict[tic]))
        print('na value',secDict[tic].isna().sum())
        secDict[tic].dropna(axis=0,inplace=True)

        print(secDict[tic].isna().sum().sum())
        print('after dropping na, length of df is:',len(secDict[tic]))

        #     400_2_1
        print('400_2_111')
        print(list(secDict[tic].columns))
        
        t=secDict[tic].apply(lambda x: TradeSim(x, 100, 2, 1), axis=1)
        ...
        print('length of df is:',len(secDict[tic]))

        #     400_1_1
        print('400_1_1')
        t=secDict[tic].apply(lambda x: TradeSim(x, 100, 1, 1), axis=1)
        t=pd.DataFrame(list(t))
        ...
        t=secDict[tic].apply(lambda x: TradeSim(x, 100, 1, 2), axis=1)
        t=pd.DataFrame(list(t))
        ...
        t=secDict[tic].apply(lambda x: TradeSim(x, 100, 0.5, 2), axis=1)
        t=pd.DataFrame(list(t))
    ...
    #### new 22
        ...
        print('lalnldnflsdf',len(secDict[tic]))
        print('doneeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee')
        secDict[tic].to_csv(f'./docker_storage/Tick_Data/Const_Resampled_2017/dolDF_const_10_BarsPerDay_wFeatsAndTgtsAndEvents/FullDataDict_400NbarLim{tic}.csv')
